...
(function () {
  ...
  /* ---------------- FAQ accordion ---------------- */
  document.querySelectorAll('.lv-faq__q').forEach((btn) => {
    btn.addEventListener('click', () => {
      const item = btn.closest('.lv-faq__item');
      const isOpen = item.classList.contains('is-open');
      // Close all others
      document.querySelectorAll('.lv-faq__item.is-open').forEach((openItem) => {
  ...
  const updateCta = () => {
    ...
    if (s) {
      cta.textContent = window.t ? window.t('nav.dashboard') : 'Open dashboard →';
      cta.href = (Auth.ROLE_META[s.role] || Auth.ROLE_META.USER).path;
    }
  };
  updateCta();
  document.addEventListener('swachlens:lang', updateCta);

  /* ---------------- Live Impact: real usage stats ---------------- */
  // The four numbers (and the numeric bits of two subtitles) come from the
  // backend so the landing page only ever shows real usage data. Until the
  // API answers (or if it never does), the values stay at 0 / "—".
  function impactEl(id) { return document.getElementById(id); }

  function setImpactNumber(id, value) {
    const el = impactEl(id);
    if (!el) return;
    el.dataset.count = String(value);
    if (el.dataset.counted) el.textContent = Number(value).toLocaleString();
  }

  function renderImpactSubtitles(wards, rate) {
    const tFn = window.t || function (k) { return k; };
    const ws = impactEl('impactWardsSub');
    if (ws) {
      ws.dataset.wards = String(wards);
      ws.textContent = tFn('land.impReportsSub', { n: wards });
    }
    const rs = impactEl('impactRateSub');
    if (rs) {
      rs.dataset.rate = String(rate);
      rs.textContent = tFn('land.impVerifiedSub', { n: rate });
    }
  }

  function applyImpactStats(data) {
    setImpactNumber('impactReports', data.reportsSubmitted || 0);
    setImpactNumber('impactVerified', data.verifiedCleanups || 0);
    setImpactNumber('impactCrew', data.activeCrew || 0);
    const resp = impactEl('impactResp');
    if (resp) {
      const hrs = data.avgResponseHours || 0;
      resp.innerHTML = hrs > 0
        ? Number(hrs).toFixed(1) + '<span class="is-unit">h</span>'
        : '—';
    }
    renderImpactSubtitles(data.wards || 0, data.resolutionRate || 0);
  }
  ...
  async function loadImpactStats() {
    ...
    try {
      ...
      const res = await fetch(base + '/stats/impact', { headers: { Accept: 'application/json' } });
      if (!res.ok) throw new Error('stats unavailable');
      data = await res.json();
    } catch (e) {
      // Backend not reachable — keep the honest zeros already in the markup.
      return;
    }
    applyImpactStats(data);
  }

  // Re-render the numeric subtitles when the language changes (values cached).
  document.addEventListener('swachlens:lang', () => {
    const ws = impactEl('impactWardsSub');
    const rs = impactEl('impactRateSub');
    renderImpactSubtitles(
      (ws && ws.dataset.wards) ? parseInt(ws.dataset.wards, 10) : 0,
      (rs && rs.dataset.rate) ? parseInt(rs.dataset.rate, 10) : 0
    );
  });
